    ...
    for blueHex in state_under_consideration["blueHexes"].keys():
        for redHex in state_under_consideration["redHexes"].keys():
            ...
            if abs(man_dist[0]) > 3:
                ...
                man_dist[0] = math.copysign(man_dist[0], sign)

            if abs(man_dist[1]) > 3:
                sign = man_dist[1]
                man_dist[1] = abs(man_dist[1]) - 7
                man_dist[1] = math.copysign(man_dist[1], sign)

      
            # diagonal or regular 
            if man_dist[0] == 0 or man_dist[1] == 0 or man_dist[0] / man_dist[1] > 0:
                distance = math.hypot(abs(man_dist[0]), abs(man_dist[1]))
    
            # vertical 
            elif man_dist[0] / man_dist[1] == -1:
                distance = abs(man_dist[0])

            elif man_dist[0] / man_dist[1] < 0:
                distance = math.sqrt(math.pow(man_dist[0], 2) + math.pow(man_dist[1], 2) - 2*abs(man_dist[0])*abs(man_dist[1])*math.cos(1/3 * math.pi))
